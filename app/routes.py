...
def dashboard():
    ...
    for server in servers:
        ...
        if gpu_info:
            # detailed_gpu_info[server.id] = gpu_info  # 存储详细信息
            server.gpu_count = len(gpu_info)
            server.gpu_usage = (
                sum(gpu["utilization"] for gpu in gpu_info) / len(gpu_info)
                if len(gpu_info) > 0
                else 0
            )
            detailed_gpu_info[server.id] = gpu_info  # 存储详细信息
    ...
    def server_sort_key(server):
        if not server.is_occupied and (
            server.gpu_usage is None or server.gpu_usage < 10
        ):
    ...
    # 获取最后一次更新时间
    last_update = gpu_usage_history.query.order_by(
        gpu_usage_history.timestamp.desc()
    ).first()
    shanghai_tz = pytz.timezone("Asia/Shanghai")
    if last_update:
        last_update_time = last_update.timestamp.replace(tzinfo=pytz.UTC).astimezone(
            shanghai_tz
        )
        next_update_time = (last_update_time + timedelta(minutes=100)).astimezone(
            shanghai_tz
        )
    else:
    ...
    return render_template(
        ...
        title="仪表板",
        servers=servers,
        detailed_gpu_info=detailed_gpu_info,  # 传递详细GPU信息
        form=form,
...
def server_management():
    ...
    if form.validate_on_submit():
        name = form.name.data
        ip = form.ip.data
        domain = form.domain.data  # 获取域名
        port = form.port.data  # 获取端口
        user = form.user.data  # 获取用户名
        ...
        if ip_range_match:
            ...
            for i in range(start, end + 1):
                ...
                # new_server = Server(name=server_name, ip=server_ip, password=password)  # 保存密码
                # new_server = Server(name=server_name, ip=server_ip)
                db.session.add(new_server)
                update_server_gpu_info(new_server)  # 传递密码
                # update_server_gpu_info(new_server)
        else:
            new_server = Server(
                name=name, ip=ip, domain=domain, port=port, user=user, password=password
            )
            # new_server = Server(name=name, ip=ip, password=password)  # 保存密码
    ...
    return render_template(
        "server_management.html", title="服务器管理", form=form, servers=servers
    )
...
def user_management():
    ...
    return render_template(
        "user_management.html", title="用户管理", form=form, users=users
    )
...
def server_detail(server_id):
    print("传进来的server_id:", server_id)
    server = Server.query.get_or_404(server_id)

    # 获取最新的 GPU 信息
    gpu_info_list = (
        gpu_info.query.filter_by(server_id=server_id)
        .order_by(gpu_info.id.desc())
        .limit(server.gpu_count)
        .all()
    )
    print("传进来的gpu_info数:", len(gpu_info_list))

    # 获取 GPU 使用率历史数据
    usage_history = (
        gpu_usage_history.query.filter_by(server_id=server_id)
        .order_by(gpu_usage_history.timestamp.desc())
        .limit(100)
        .all()
    )
    ...
    shanghai_tz = pytz.timezone("Asia/Shanghai")
    timestamps = [
        # fix: should use UTC timestamp
        h.timestamp.replace(tzinfo=shanghai_tz)
        .astimezone(shanghai_tz)
        .strftime("%Y-%m-%d %H:%M:%S")
        for h in reversed(usage_history)
    ]
